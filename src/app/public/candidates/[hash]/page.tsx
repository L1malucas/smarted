...
import { getPublicCandidateDetailsAction } from "@/infrastructure/actions/public-actions"
...
export default function PublicCandidatesPage() {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        setLoading(true);
        const result = await getPublicCandidateDetailsAction(params.hash as string);
        if (result.success && result.data) {
          setJobTitle(result.data.jobTitle);
          setCandidates(result.data.candidates);
          setError(null);
        } else {
          setError(result.error || "Não foi possível carregar os dados dos candidatos. O link pode ter expirado ou o recurso não existe mais.");
        }
      } catch (err) {
        setError(
